...
public interface TransactionRepository extends JpaRepository<Transaction, Long>{
    ...
    @Query("SELECT CASE :dateFormat WHEN 'Day' THEN DATE_FORMAT(t.timestamp, '%Y-%m-%d') WHEN 'Month' THEN DATE_FORMAT(t.timestamp, '%Y-%m') WHEN 'Year' THEN DATE_FORMAT(t.timestamp, '%Y') END, SUM(t.value) FROM Transaction t WHERE t.userId = :userId AND t.timestamp BETWEEN :startDate AND :endDate " +
    ...
    @Query("SELECT t.timestamp, SUM(t.value) FROM Transaction t WHERE t.userId = :userId AND t.timestamp BETWEEN :startDate AND :endDate " +
